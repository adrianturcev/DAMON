            ...
            for (let i = 0, c = tree.children.length; i < c; i++) {
              ...
              if (tree.children[i].content.length == 0) {
                ...
                if (tree.children[i].children.length > 0) {
                  ...
                  if (indexPrefixKeys) {
                    jsonMap.set(i + "-", /* @__PURE__ */ new Map());
                  } else {
                ...
                try {
                  if (/^.*: +\[/.test(text) && text[text.length - 1] == "]") {
                    if (/: +\[ *\]$/.test(text)) {
                      ...
                      if (tree.children[i].children.length > 0) {
                        if (indexPrefixKeys) {
                          _recurse(tree.children[i], jsonMap.get(i + "-" + key));
                        } else {
                      ...
                      if (arrayOfPrimitives) {
                        if (indexPrefixKeys) {
                          jsonMap.set(i + "-" + shortestPossibleKey, longestPossibleArray);
                        } else {
                        ...
                        if (indexPrefixKeys) {
                          jsonMap.damonInlineArrays.push(i + "-" + shortestPossibleKey);
                        } else {
                    ...
                    if (indexPrefixKeys) {
                      jsonMap.set(i + "-" + key, /* @__PURE__ */ new Map());
                      _recurse(tree.children[i], jsonMap.get(i + "-" + key));
                    } else {
                      ...
                      if (validValue) {
                        ...
                        if ($.pedantic) {
                          ...
                          if (tree.children[i].children.length > 0) {
                            ...
                            if (indexPrefixKeys) {
                              jsonMap.implicitMaps.push(i + "-" + key);
                              _recurse(tree.children[i], jsonMap.get(i + "-" + key));
                            } else {
                            ...
                            if (indexPrefixKeys) {
                              jsonMap.implicitNulls.push(i + "-" + key);
                            } else {
            ...
            if (Array.isArray(object)) {
              for (let i = 0, c = object.length; i < c; i++) {
                if (Array.isArray(object[i])) {
                  ...
                  damonObject[i] = /* @__PURE__ */ new Map();
                  recurse(object[i], damonObject[i]);
                } else {
              ...
              let keys = Object.keys(object), range = new Array(keys.length);
              for (let i = 0, c = keys.length; i < c; i++) {
                let keyArray = keys[i].split("-"), keyIndex = keyArray[0] * 1;
                range[keyIndex] = keyArray.slice(1).join("-");
              }
              keys = range;
              for (let i = 0, c = keys.length; i < c; i++) {
                let value = object[i + "-" + keys[i]], k = keys[i];
                if (Array.isArray(value)) {
                  damonObject.set(k, []);
                  recurse(value, damonObject.get(k));
                } else if (typeof value === "object" && value !== null) {
                  damonObject.set(k, /* @__PURE__ */ new Map());
                  recurse(value, damonObject.get(k));
                } else {
